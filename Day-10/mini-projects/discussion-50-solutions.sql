...
--- Tasks
    ...
    --- Task 1.2: Total Quantity Sold by Product Line - Determine the total quantity sold for each product line.
        select productLine,sum(quantityOrdered) as total_quantity_ordered from orderdetails join products using(productCode) group by productLine;

    --- Task 1.3: Office Sales Analysis - Determine the total sales for each office, including office city, number of orders processed, and total sales amount.
...
--- Tasks
    --- Task 2.1 High-Value Order Analysis - Identify offices with an average order value greater than a certain threshold. Include office city, average order value, and total number of orders.
    ...
    --- Task 2.2 Product Line Performance Filter - Filter product lines that have an average product sale price above a specific value.
        select productLine, avg(MSRP) from products group by productLine having avg(MSRP)>100;

    --- Task 2.3 Average Sale Amount by Product Line with Filtering - Find product lines with an average sale amount above a specified threshold.
...
---Tasks
    --- Task 3.1 Continent Analysis: For each continent, find the average population and total GDP. Filter out continents with an average population below a certain threshold.
        ...
        select continent, avg(population) as average_population from country group by continent having avg(population)<2000000;
    ...
    --- Task 3.2 Language Diversity: Identify countries with more than a specific number of official languages and display the country name, number of official languages, and total population.
        select Name as country_name, count(IsOfficial='T') as no_official_languages, sum(population) as total_population from country join countrylanguage on country.code=countrylanguage.countrycode group by name having count(IsOfficial='T')>10;
