--- Write a query to find the addresses (location_id, street_address, city, state_province, country_name) of all the departments.
    select location_id, street_address, city, state_province, country_name, department_name from locations join countries using(country_id) join departments using(location_id) group by department_id;

--- Write a query to find the name (first_name, last name), department ID, and department name of all the employees.
    select concat(first_name," ",last_name) as name_of_employee, department_id, department_name from employees join departments using(department_id);

--- Write a query to find the name (first_name, last_name), job, department ID, and name of the employees who work in London.
    select concat(first_name," ",last_name) as name_of_employee, job_title, department_id from employees join jobs using(job_id) join departments using(department_id) join locations using(location_id) where city="London";
...
--- Write a query to get the department name and number of employees in the department.
    select department_name, count(employee_id) as no_of_employees from departments join employees using(department_id) group by department_name order by count(EMPLOYEE_ID);
...
--- Write a query to display the department name, manager name, and city.
    select department_name, concat(first_name," ",last_name) as manager_name, city from departments join employees using(department_id) join locations using(location_id) where employees.EMPLOYEE_ID=departments.MANAGER_ID;
...
--- Write a query to display the job history of any employee who is currently drawing more than 10000 of salary.
    select job_history.* from job_history join employees using(job_id) where salary>10000;
...
--- Write a query to display department name, name (first_name, last_name), hire date, the salary of the manager for all managers whose experience is more than 15 years.
    select department_name, concat(first_name," ",last_name) as manager_name, hire_date, salary from employees join departments using(department_id) join job_history using(employee_id) where employee_id=departments.manager_id and end_date-start_date>15; 