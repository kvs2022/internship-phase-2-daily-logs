...
--- 1. Write a query to find customers whose payments are greater than the average payment using a subquery.
    select * from payments where amount > (select avg(amount) from payments where customers.customerNumber = payments.customerNumber);
...
--- 4. Unlike a standalone subquery, a correlated subquery is a subquery that uses the data from the outer query. In other words, a correlated subquery depends on the outer query. A correlated subquery is evaluated once for each row in the outer query.
    ...
    select productName, buyprices from products where buyprice > (select avg(buyprice) from products);
...
--- 6. When a subquery is used with the EXISTS or NOT EXISTS operator, a subquery returns a Boolean value of TRUE or FALSE. 
    -- The following query illustrates a subquery used with the EXISTS operator: In the query above, if the subquery returns any rows, EXISTS subquery returns TRUE, otherwise, it returns FALSE.
...
--- 9. Suppose you have to label the customers who bought products in 2003 into 3 groups: platinum, gold, and silver with the following conditions:
    --- a. Platinum customers who have orders with the volume greater than 100K.
    --- b. Gold customers who have orders with the volume between 10K and 100K.
    --- c. Silver customers who have orders with the volume less than 10K.
    --- d. To form this query, you first need to put each customer into the respective group using CASE expression and GROUP BY to display the following:
